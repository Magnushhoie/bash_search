...
search_file() {
filename=$1
  grep --color=always -B 10 -A 20 "$2" "$filename" |
  grep --color=always -B 9 -A 19 "$3" |
  grep --color=always -B 8 -A 18 "$4" |
  grep --color=always -B 7 -A 17 "$5" |
  grep --color=always -B 6 -A 16 "$6" |
  grep --color=always -B 5 -A 15 "$7" |
  less -R
}
...
function ref() # Search references.txt. Accepts search terms and/or files in search folder. E.g. ref datascience.txt Randomforest
{
    filename=$(ref_get_notefile)
     ...
     # Open file with less if no search terms. Otherwise search the file
     if [ -z "$1" ]; then
        less -R $filename
     else
...
# Notefile is references.txt, unless another file found from first argument
    ...
    if [ -z $1 ];
    then vim "$filename"
    else vim +":set hlsearch" +/"$@" $filename
    fi
}
...
eref_get_notefile() # Helper function for ref functions
{
# Searches note folder for notefile. Default is references.txt
# Alternative name given if first argument matches that basename excluding file extension
# New filename given if first argument ends in .txt
# List of available files given if first argument is "list"
    ...
    if [ ${#firstword} -ge 4 ];
        then if [ ${firstword: -4} = ".txt" ];
            then notefile="$eref_folder"/$firstword
        fi
    fi
...
function eref() # Search references.txt. Accepts search terms and/or files in search folder. E.g. ref datascience.txt Randomforest
{
    filename="$(eref_get_notefile)"

     # Check for alternative filename as first argument
     # If so, shift arguments so second+ becomes keyword
     alternative_filename=$(eref_get_notefile $1)
     if [ "$filename" != "$alternative_filename" ];
         then filename="$alternative_filename"
         shift
     fi
...
function erefv() # Search and edit references.txt in vim. Can create new files, e.g. refv datascience.txt
{
# Opens up vim at first mention of keyword(s)
# Notefile is references.txt, unless another file found from first argument
     filename="$(eref_get_notefile)"
    ...
    if [ $1 = "-h" -o $1 = "-help" -o $1 = "list" ];
        then echo -e "\nAvailable files:"
             for each in $(eref_get_notefile $1);
             do ls -ltrh $each; done
